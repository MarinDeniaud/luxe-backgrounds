...
def plotAverageJitterPerBunchID(xfel_data, bpms=['BPMI.1910.TL', 'BPMI.1925.TL', 'BPMI.1930.TL', 'BPMI.1939.TL'],
                                inSubplots=False, figsize=[12, 5]):
    bunchIDs, sx_mean, sy_mean = xfel_data.calcAverageJitterPerBunchID(bpms=bpms)
    if not inSubplots:
        _plt.figure(figsize=figsize)
        _plt.xlabel(r"$s$ [m]")
    labelT1 = 'XTD1'
    labelT2 = 'XTD2'
    for bunchT1 in xfel_data.bunchIDs_XTD1[::10]:
        _plt.axvline(bunchT1, color='C2', alpha=0.3, label=labelT1)
        labelT1 = None
    for bunchT2 in xfel_data.bunchIDs_XTD2[::5]:
...
def plotOneAlong(ax, df_reduced, slist, xlabel, coord='X', label='x', unit=r"$\rm \mu m$", minCut=None, maxCut=None):
    ...
    ax.plot(slist[minCut:maxCut], X_cut, color='C0',
            label=r"$\overline{{{}}}$ = {:.3f} {}".format(label, _np.mean(X_cut), unit))
...
def PlotAlong(ax_x, ax_y, df_reduced, bpms=None, trains=None, bunches=None, minCut=None, maxCut=None):
    ...
    ax_x.plot(slist[minCut:maxCut], X_cut, color='C0', label=r"$\overline{{x}}$ = {:.2f} $\rm \mu m$".format(_np.mean(X_cut)))
    ax_y.plot(slist[minCut:maxCut], Y_cut, color='C1', label=r"$\overline{{y}}$ = {:.2f} $\rm \mu m$".format(_np.mean(Y_cut)))
...
def plotTime(df, bpms=None, trains=None, bunches=None, minCut=None, maxCut=None, figsize=[9, 6]):
    df_reduced = XFEL_BPM.reduceDFbyBPMTrainBunchByIndex(df, bpms=bpms, trains=trains, bunches=bunches)
    if bpms is None and trains is not None and bunches is not None:
        xlabel = 'BPMs'
        slist = df_reduced.index.get_level_values('BPM')
    ...
    plotOneHist(ax_right, df_reduced, coord='TimeS', label="t", unit='ps', minCut=minCut, maxCut=maxCut)
